...
export default {
  ...
  created() {
    // tab标签页的切换是可以触发生命周期钩子函数的,所以接口可以在created钩子函数中调用
    // v-if控制组件的显示和隐藏时,也会触发生命周期钩子函数 v-if是对节点进行删除和创建,所以会触发生命周期钩子函数
  ...
  methods: {
    ...
    async getUserDetailById() {
      ...
      if (this.userInfo.staffPhoto && this.userInfo.staffPhoto.trim()) {
        // 有值表示 已经有一张上传成功的图片,就将这个用户的图片传给子组件的fileList数组里面,控制展示的图片
        // 这里也需要添加upload: true
    ...
    async saveUser() {
      ...
      // 由于接口问题必须给一个有空格的字符串才能过去
      // 如果图片上传成功则调用接口,将图片地址作为参数传递给userInfo
